...
``tests/integration/test_m4_parity.py`` 的 compare_* 函数）。已知 produced_qty 因 RNG 路径
差异（旧 DuckDB vs 新 pandas）天然发散、少数行产能拆分天差一行，按既有 parity
结论**只报告、不硬失败**；con_planned_qty / changeover_id / mu_loss 等allocator
产物才是重构 parity 的主体。

运行：
    pytest tests/integration/test_m4_refactor_integration.py -q
    # 缩短区间：
    M4_TEST_START=2025-12-15 M4_TEST_END=2025-12-19 pytest tests/integration/test_m4_refactor_integration.py -q
    # 或脚本：
    python tests/integration/test_m4_refactor_integration.py --start 2025-12-15 --end 2025-12-19
"""
...
from tests.integration.test_m4_parity import (
...
def test_m4_refactor_integration():
    # 测试目的：验证“m4、refactor、integration”场景下生产排程、产能分配与生产结果的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `run_integrated_simulation_m4()`，再通过 2 个断言核对结果、状态或异常条件。
...
def test_m4_refactor_parity():
    # 测试目的：验证“m4、refactor、parity”场景下生产排程、产能分配与生产结果的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `run_integrated_simulation_m4()`，再通过 1 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止生产排程、产能分配与生产结果的一致性在重构、引擎切换或跨日运行时发生静默偏差。
...
def test_m4_refactor_determinism():
    # 测试目的：验证“m4、refactor、determinism”场景下生产排程、产能分配与生产结果的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `run_integrated_simulation_m4()`，再通过 1 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止生产排程、产能分配与生产结果的一致性在重构、引擎切换或跨日运行时发生静默偏差。
