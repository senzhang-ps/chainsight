...
from tests.diagnostics.test_full_integration_db_config_diagnosis import _db, _write_db_config_excel
...
def test_m6_cd_five_day_full_integration() -> None:
    # 测试目的：验证“m6、cd、five、day、full、integration”场景下物流执行、车辆分配与交付结果的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `mkdir()`，再通过 3 个断言核对结果、状态或异常条件。
