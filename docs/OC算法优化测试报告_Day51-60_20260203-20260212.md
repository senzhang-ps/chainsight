...
| **Src** | 2046秒 (34.1分钟) | 204.6秒 |
| **DB** | 1063秒 (17.7分钟) | 106.3秒 |

### 2.2 性能提升比例
| 对比项 | 加速倍数 | 时间减少百分比 |
|--------|----------|----------------|
| **Src vs Dev** | **6.06x** | 83.5% |
| **DB vs Dev** | **11.67x** | 91.4% |
| **DB vs Src** | **1.92x** | 48.0% |

### 2.3 各模块平均耗时对比（秒/天）
| 模块 | Dev | Src | DB | Src加速 | DB加速 |
|------|-----|-----|-----|---------|--------|
| **M1 订单生成** | ~362.7 | ~42.1 | ~38.6 | 8.62x | 9.40x |
| **M3 净需求计算** | ~337.8 | ~47.3 | ~20.3 | 7.14x | 16.64x |
| **M5 部署规划** | ~508.5 | ~101.1 | ~34.9 | 5.03x | 14.57x |

### 2.4 每日详细耗时（区间最后一天）
#### Day 60 (2026-02-12) 各模块耗时（秒）
| 模块 | Dev | Src | DB |
|------|-----|-----|-----|
| M1 订单生成 | ~347.0 | ~41.0 | ~39.0 |
| M3 净需求计算 | ~337.0 | ~45.0 | ~20.0 |
| M5 部署规划 | ~517.0 | ~95.0 | ~33.0 |
| M6 物流执行 | ~2.0 | ~1.0 | ~1.0 |
...
|------|------|---------|---------|--------|------------|-----------|-----------|
| Module1 | 订单日志 (OrderLog) | 45,836 | 45,836 | 45,836 | PASS | PASS | PASS |
...
| **合计** | | **45,836** | **45,836** | **45,836** | **10/10天PASS** | **10/10天PASS** | **10/10天PASS** |
...
验证覆盖（本区间）：
- Module1 / 订单日志 (OrderLog): 45,836行 (Dev vs Src PASS, Dev vs DB PASS, Src vs DB PASS)
...
| 指标 | Dev | Src | DB | 最佳提升 |
|------|-----|-----|-----|----------|
| 区间总时间 | 206.8分钟 | 34.1分钟 | 17.7分钟 | 91.4% |
| M1 订单生成平均耗时/天 | ~362.7秒 | ~42.1秒 | ~38.6秒 | 89.4% |
| M3 净需求计算平均耗时/天 | ~337.8秒 | ~47.3秒 | ~20.3秒 | 94.0% |
| M5 部署规划平均耗时/天 | ~508.5秒 | ~101.1秒 | ~34.9秒 | 93.1% |
