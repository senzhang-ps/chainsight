...
# -*- coding: utf-8 -*-
"""
bc_3way_content_compare_v2.py

三版本（Dev/Src/DB）逐日逐Sheet内容级对比工具 — BC_S5 版本 (v2)。
...
修复:
  - module1/Summary: total_orders Dev/Src/DB 统一用 OrderLog 当日新增行数
  - normalize_value: 处理 inf 值
  - 使用频率比较 (Counter) 避免排序依赖

输出:
  - JSON格式的全量对比结果 (tools/bc_3way_content_results.json)
  - 控制台摘要

Usage:
    python tools/bc_3way_content_compare_v2.py [--days 1-10] [--module module1]
"""
...
def get_daily_order_count(filepath: Path, date_str: str) -> int | None:
    ...
    Dev/Src 的 xlsx 每天包含截至当天的所有订单 (累积), DB 的 sim_date 分区同样
    存储的是截至当天的累积行数, 因此直接取 OrderLog 总行数即可保持一致。
    """
...
# Column names that represent location codes
...
def compare_two_dfs(
    ...
    # Sort by all columns before cumcount to ensure stable dup_idx alignment
    ...
    for col in avail_compare:
        ...
        if both_numeric:
            try:
                # Handle inf values: replace inf with a large sentinel
                v1_vals = v1.fillna(0).astype(float).values
                v2_vals = v2.fillna(0).astype(float).values
                # inf == inf should be True
                both_inf = np.isinf(v1_vals) & np.isinf(v2_vals) & (np.sign(v1_vals) == np.sign(v2_vals))
                mask = ~np.isclose(v1_vals, v2_vals, atol=tolerance, equal_nan=True)
                mask = mask & ~both_inf  # Don't flag matching infinities
            except Exception:
    ...
    if diff_details:
        result["diff_details"] = diff_details[:20]
...
def run_comparison(day_range: range, module_filter: str | None = None,
    ...
    for mapping in SHEET_MAPPINGS:
        ...
        for day_num in day_range:
            ...
            # ── 修正 module1/Summary 的 total_orders ──
            # Dev/Src/DB 统一用 OrderLog 总行数替换 (xlsx 是累积数据)
            if comp_key == "module1/Summary":
                ...
                # DB total_orders 也统一：从DB的orderlog取当日行数
                if db_df is not None and conn is not None:
                    try:
                        cur = conn.cursor()
                        cur.execute("SELECT COUNT(*) FROM module1_output_orderlog WHERE sim_date = %s", (ds,))
                        db_daily_orders = cur.fetchone()[0]
                        db_df["total_orders"] = db_daily_orders
                    except Exception:
                        pass
...
def print_summary(results: dict) -> None:
    ...
    print("\n" + "=" * 130)
    print("3-WAY COMPARISON SUMMARY (BC_S5)")
    print("=" * 130)
    print(f"{'Sheet':<35} {'Dev Rows':>10} {'Src Rows':>10} {'DB Rows':>10} {'Dev/Src':>14} {'Dev/DB':>14} {'Src/DB':>14}")
    print("-" * 130)
    ...
    for comp_key, data in results["comparisons"].items():
        ...
        print(f"{comp_key:<35} {data['total_dev_rows']:>10,} {data['total_src_rows']:>10,} "
              f"{data['total_db_rows']:>10,} {ds_status:>14} {db_status:>14} {sb_status:>14}")
...
def main():
    parser = argparse.ArgumentParser(description="BC_S5 3-way content comparison (v2)")
    parser.add_argument("--days", type=str, default=None,
                        help="Day range, e.g. '1-10' or '1-87' (default: all 87)")
    parser.add_argument("--module", type=str, default=None,
                        help="Filter to single module, e.g. 'module1'")
    parser.add_argument("--output", type=str, default=None,
                        help="Output JSON path (default: tools/bc_3way_content_results.json)")
    ...
    out_path = Path(args.output) if args.output else PROJECT_ROOT / "tools" / "bc_3way_content_results.json"
