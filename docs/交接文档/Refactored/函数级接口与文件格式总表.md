...
#### `main(argv=None) -> int`
- 功能：CLI 总入口，解析参数、选择本地或数据库模式、初始化输出目录和日志。
...
- 作用：把 `demand_planning` 子包公开函数重新导出，保持老接口不变。
- 重点 re-export：`run_daily_order_generation`, `generate_daily_orders`, `generate_shipment_with_inventory_check`, `expand_forecast_to_days_integer_split`, `apply_dps`, `apply_supply_choice`, `load_config`。

### `src/modules/module3.py`
- 作用：把 `mrp_planning` 核心 API 重新导出。
- 重点 re-export：`run_integrated_mode`, `run_mrp_layered_simulation_daily`, `calculate_daily_net_demand`, `assign_location_layers`, `determine_lead_time`。

### `src/modules/module4.py`
- 作用：把 `production_planning` 主要函数和类型重新导出。
- 重点 re-export：`run_daily_production_planning`, `DailyProductionPlanner`, `build_unconstrained_plan_for_single_day`, `centralized_capacity_allocation_with_changeover`, `simulate_production`。

### `src/modules/module5.py`
- 作用：把 `deployment_planning` 主要入口重新导出。
- 重点 re-export：`main`, `load_integrated_config`, `collect_node_demands`, `apply_grouped_moq_rv`, `apply_priority_allocation_vectorized`, `push_softpush_allocation`。

### `src/modules/module6.py`
- 作用：既是兼容层，也是实际的 M6 主流程封装文件。
...
1. `src/core/run.py`
2. `src/core/main_integration.py`
...
- `module1.py`、`module3.py`、`module4.py`、`module5.py` 主要是兼容层，真正的逻辑都在对应子包里。
- `module6.py` 兼容层属性较弱，它本身仍承载了大量实际主流程逻辑，所以在维护优先级上更高。
- 如果你要进一步把这份手册升级成“逐文件逐函数完全展开版”，下一步最值得继续深挖的是：
  ...
  4. `src/modules/logistics_execution/module6.py`
...
### 28.4 `src/modules/module6.py` 深度说明
...
5. `module6.py` 中 route 顺序、truck type 顺序、第一轮/第二轮装载、UID 去重顺序都会影响最终结果。
