...
keep = [c for c in ['material', 'category_en'] if c in mapping.columns]
mapping = mapping[keep].drop_duplicates(subset=['material']).copy()
mapping['category_en'] = mapping['category_en'].replace({'': '(unmapped)', 'nan': '(unmapped)', 'None': '(unmapped)'}).fillna('(unmapped)')
...
out1 = ROOT / 'workspace' / 'south-space-rccp-jason26' / 'scenarios' / 'production-cycle' / 'results' / 'rccp_by_month_location_category_en_focus_locations.csv'
out2 = ROOT / 'workspace' / 'south-space-rccp-jason26' / 'scenarios' / 'production-cycle' / 'results' / 'rccp_by_month_category_en_focus_locations.csv'
out1.parent.mkdir(parents=True, exist_ok=True)
by_loc_cat_peak.to_csv(out1, index=False, encoding='utf-8-sig')
by_cat_peak.to_csv(out2, index=False, encoding='utf-8-sig')
print('BY_MONTH_BY_LOCATION_BY_CATEGORY_EN')
print(by_loc_cat_peak.head(40).to_string(index=False))
print(f'\\nrows={len(by_loc_cat_peak)}\\nout={out1}')
print('\\nBY_MONTH_BY_CATEGORY_EN')
print(by_cat_peak.to_string(index=False))
print(f'\\nrows={len(by_cat_peak)}\\nout={out2}')
