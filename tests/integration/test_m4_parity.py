...
运行
----
    pytest tests/integration/test_m4_parity.py -v
    python tests/integration/test_m4_parity.py
"""
...
def test_m4_parity_db():
    # 测试目的：验证“m4、parity、db”场景下生产排程、产能分配与生产结果的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `run_parity()`，再通过 1 个断言核对结果、状态或异常条件。
