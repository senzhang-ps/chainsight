...
Scenario: baseline | run_id: db_baseline_20260616_154430
...
SCENARIO = "baseline"
RUN_ID = "db_baseline_20260616_154430"
...
def pull_postgres():
    with pg_connect() as conn:
        ...
        # KPI6 inputs ------------------------------------------------------------
        # APQ = total constrained qty / number of production runs. A run begins with
        # a changeover, so each plan row with a non-null changeover_id is one run; the
        # 67 null-changeover_id rows are continuations of the current run (no preceding
        # changeover) and are not counted as separate runs. Grain: material x line.
        prod_runs = q(conn, """
    ...
    return (orders, ships, line_map, changeover, co_def, stock_me, fcst_weekly,
            dc_info, prod_plan, prd_rate, prod_runs)
...
def compute_apq(prod_runs, base):
    """APQ = average constrained planned qty per production run.
    ...
    APQ = total con_planned_qty / run count, reported by production line and by
    material. Category is attached for context using the same resolution as TOF.
    """
    pr = prod_runs.copy()
    ...
    def apq_frame(g, keys):
        out = g.groupby(keys, as_index=False).agg(
            con_qty=("con_qty", "sum"), runs=("runs", "sum"))
        out["apq"] = (out["con_qty"] / out["runs"].replace(0, np.nan)).round(1)
    ...
    pm["apq"] = (pm["con_qty"] / pm["runs"].replace(0, np.nan)).round(1)
    pm["con_qty"] = pm["con_qty"].round(1)
    apq_by_material = pm[["material", "line", "category", "con_qty", "runs", "apq"]] \
        .sort_values("con_qty", ascending=False)
    ...
    info = {
        ...
        "total_runs": tot_runs,
        "apq_overall": round(tot_qty / tot_runs, 1) if tot_runs else float("nan"),
...
def build_charts(cfr_by_cat, cfr_line_total, changeover, dfc_by_cat, dfc_by_line,
    ...
    try:
        ...
        # Chart 10: APQ (avg constrained qty per production run) by line
        ab = apq_by_line.sort_values("apq", ascending=False)
        fig, ax = plt.subplots(figsize=(7, 3.4))
        ax.bar(ab["line"], ab["apq"], color="#2563eb", width=0.7)
        ax.set_ylabel("APQ (units / run)"); ax.set_xlabel("")
        ax.set_title("APQ \u2014 avg constrained planned qty per production run, by line")
...
def main():
    ...
    run_id = datetime.now().strftime("%Y%m%d-%H%M") + "-baseline-tof-changeover-dfc-prodtime-apq-msu"
    ...
    (orders, ships, line_map, changeover, co_def, stock_me, fcst_weekly, dc_info,
     prod_plan, prd_rate, prod_runs) = pull_postgres()
    ...
    # KPI6: APQ (avg constrained planned qty per production run)
    apq_by_line, apq_by_material, apq_info = compute_apq(prod_runs, base)
    ...
    prod_materials = sorted(prod_runs["material"].astype(str).unique())
    ...
    meta = {
        ...
        "msu_n_suf": su_info["n_suf"],
        "msu_n_dbx": su_info["n_dbx"],
        "msu_n_manual": su_info["n_manual"],
        "msu_n_missing": su_info["n_missing"],
    ...
    print(f"APQ: total con qty {apq_info['total_con_qty']:,.0f} / {apq_info['total_runs']} runs "
          f"= {apq_info['apq_overall']:,.1f} units/run | {apq_info['n_materials']} materials")
