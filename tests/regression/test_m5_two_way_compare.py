...
运行：
    conda run -n work pytest tests/regression/test_m5_two_way_compare.py -s -q
"""
...
def test_m5_three_way_compare_replays_five_real_business_days(comparison_data):
    # 测试目的：验证“m5、three、way、compare、replays、five、real、business、days”场景下部署计划、优先级分配与供给池扣减的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `exists()`，再通过 8 个断言核对结果、状态或异常条件。
...
def test_m5_three_way_compare_records_actual_execution_timing(comparison_data):
    # 测试目的：验证“m5、three、way、compare、records、actual、execution、timing”场景下部署计划、优先级分配与供给池扣减的一致性。
...
def test_m5_polars_real_data_performance_only():
    # 测试目的：验证“m5、polars、real、data、performance、only”场景下部署计划、优先级分配与供给池扣减的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `_test_m5_real_data_performance_only()`，再验证调用完成后的可观察结果。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止部署计划、优先级分配与供给池扣减的一致性在重构、引擎切换或跨日运行时发生静默偏差。
...
def test_m5_pandas_real_data_performance_only():
    # 测试目的：验证“m5、pandas、real、data、performance、only”场景下部署计划、优先级分配与供给池扣减的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `_test_m5_real_data_performance_only()`，再验证调用完成后的可观察结果。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止部署计划、优先级分配与供给池扣减的一致性在重构、引擎切换或跨日运行时发生静默偏差。
...
def test_m5_pandas_polars_real_data_difference_diagnostic():
    # 测试目的：验证“m5、pandas、polars、real、data、difference、diagnostic”场景下部署计划、优先级分配与供给池扣减的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `mkdir()`，再通过 5 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止部署计划、优先级分配与供给池扣减的一致性在重构、引擎切换或跨日运行时发生静默偏差。
...
def test_m5_pandas_polars_real_data_state_propagation_parity():
    # 测试目的：验证“m5、pandas、polars、real、data、state、propagation、parity”场景下部署计划、优先级分配与供给池扣减的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `mkdir()`，再通过 4 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止部署计划、优先级分配与供给池扣减的一致性在重构、引擎切换或跨日运行时发生静默偏差。
...
def test_m5_pandas_polars_first_divergent_step_diagnostic():
    # 测试目的：验证“m5、pandas、polars、first、divergent、step、diagnostic”场景下部署计划、优先级分配与供给池扣减的一致性。
