...
| **Src** | 1867秒 (31.1分钟) | 186.7秒 |
| **DB** | 1006秒 (16.8分钟) | 100.6秒 |

### 2.2 性能提升比例
| 对比项 | 加速倍数 | 时间减少百分比 |
|--------|----------|----------------|
| **Src vs Dev** | **6.55x** | 84.7% |
| **DB vs Dev** | **12.16x** | 91.8% |
| **DB vs Src** | **1.86x** | 46.1% |

### 2.3 各模块平均耗时对比（秒/天）
| 模块 | Dev | Src | DB | Src加速 | DB加速 |
|------|-----|-----|-----|---------|--------|
| **M1 订单生成** | ~344.4 | ~39.9 | ~37.7 | 8.63x | 9.14x |
| **M3 净需求计算** | ~336.3 | ~43.7 | ~20.4 | 7.70x | 16.49x |
| **M5 部署规划** | ~518.0 | ~93.1 | ~33.1 | 5.56x | 15.65x |

### 2.4 每日详细耗时（区间最后一天）
#### Day 70 (2026-02-22) 各模块耗时（秒）
| 模块 | Dev | Src | DB |
|------|-----|-----|-----|
| M1 订单生成 | ~365.0 | ~39.0 | ~38.0 |
| M3 净需求计算 | ~333.0 | ~41.0 | ~21.0 |
| M5 部署规划 | ~501.0 | ~89.0 | ~32.0 |
| M6 物流执行 | ~2.0 | ~1.0 | ~1.0 |
...
|------|------|---------|---------|--------|------------|-----------|-----------|
| Module1 | 订单日志 (OrderLog) | 45,793 | 45,793 | 45,793 | PASS | PASS | PASS |
...
| **合计** | | **45,793** | **45,793** | **45,793** | **10/10天PASS** | **10/10天PASS** | **10/10天PASS** |
...
验证覆盖（本区间）：
- Module1 / 订单日志 (OrderLog): 45,793行 (Dev vs Src PASS, Dev vs DB PASS, Src vs DB PASS)
...
| 指标 | Dev | Src | DB | 最佳提升 |
|------|-----|-----|-----|----------|
| 区间总时间 | 203.9分钟 | 31.1分钟 | 16.8分钟 | 91.8% |
| M1 订单生成平均耗时/天 | ~344.4秒 | ~39.9秒 | ~37.7秒 | 89.1% |
| M3 净需求计算平均耗时/天 | ~336.3秒 | ~43.7秒 | ~20.4秒 | 93.9% |
| M5 部署规划平均耗时/天 | ~518.0秒 | ~93.1秒 | ~33.1秒 | 93.6% |
