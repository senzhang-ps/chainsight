...
def test_summary_tables_are_registered_for_migration() -> None:
    # 测试目的：验证“summary、tables、are、registered、for、migration”场景下日末状态视图与汇总数据的正确结转。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `values()`，再通过 2 个断言核对结果、状态或异常条件。
...
def test_state_context_builds_all_registered_summary_outputs() -> None:
    # 测试目的：验证“state、context、builds、all、registered、summary、outputs”场景下日末状态视图与汇总数据的正确结转。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `StateContext()`，再通过 5 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止日末状态视图与汇总数据的正确结转在重构、引擎切换或跨日运行时发生静默偏差。
...
def test_order_summary_prefers_m1_daily_persistence_payload() -> None:
    # 测试目的：验证“order、summary、prefers、m1、daily、persistence、payload”场景下订单生成、库存消耗与发运数据的一致性。
...
def test_order_summary_aggregates_source_days_at_legacy_business_grain() -> None:
    # 测试目的：验证“order、summary、aggregates、source、days、at、legacy、business、grain”场景下日末状态视图与汇总数据的正确结转。
...
def test_summary_persistence_uses_only_registered_summary_tables() -> None:
    # 测试目的：验证“summary、persistence、uses、only、registered、summary、tables”场景下日末状态视图与汇总数据的正确结转。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `_FakeOrchestrator()`，再通过 6 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止日末状态视图与汇总数据的正确结转在重构、引擎切换或跨日运行时发生静默偏差。
...
def test_daily_summary_persistence_uses_source_simulation_date() -> None:
    # 测试目的：验证“daily、summary、persistence、uses、source、simulation、date”场景下日末状态视图与汇总数据的正确结转。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `_FakeOrchestrator()`，再通过 1 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止日末状态视图与汇总数据的正确结转在重构、引擎切换或跨日运行时发生静默偏差。
