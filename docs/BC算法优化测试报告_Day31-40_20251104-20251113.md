...
| **Src** | 308秒 (5.1分钟) | 30.8秒 |
| **DB** | 176秒 (2.9分钟) | 17.6秒 |

### 2.2 性能提升比例
| 对比项 | 加速倍数 | 时间减少百分比 |
|--------|----------|----------------|
| **Src vs Dev** | **3.57x** | 72.0% |
| **DB vs Dev** | **6.25x** | 84.0% |
| **DB vs Src** | **1.75x** | 42.9% |

### 2.3 各模块平均耗时对比（秒/天）
| 模块 | Dev | Src | DB | Src加速 | DB加速 |
|------|-----|-----|-----|---------|--------|
| **M1 订单生成** | ~20.6 | ~6.5 | ~6.4 | 3.17x | 3.22x |
| **M3 净需求计算** | ~25.0 | ~5.7 | ~2.3 | 4.39x | 10.87x |
| **M5 部署规划** | ~58.2 | ~15.7 | ~6.1 | 3.71x | 9.54x |
...
| M3 净需求计算 | ~22.0 | ~6.0 | ~2.0 |
| M5 部署规划 | ~50.0 | ~16.0 | ~6.0 |
| M6 物流执行 | ~2.0 | ~1.0 | ~1.0 |
...
| 指标 | Dev | Src | DB | 最佳提升 |
|------|-----|-----|-----|----------|
| 区间总时间 | 18.3分钟 | 5.1分钟 | 2.9分钟 | 84.0% |
| M1 订单生成平均耗时/天 | ~20.6秒 | ~6.5秒 | ~6.4秒 | 68.9% |
| M3 净需求计算平均耗时/天 | ~25.0秒 | ~5.7秒 | ~2.3秒 | 90.8% |
| M5 部署规划平均耗时/天 | ~58.2秒 | ~15.7秒 | ~6.1秒 | 89.5% |
