...
def apply_dps(df: pd.DataFrame, dps_cfg: pd.DataFrame) -> pd.DataFrame:
    ...
    # 合并DPS配置（归一化后再 merge，避免 material 类型不匹配）
    cols = ['material', 'location', 'dps_location', 'dps_percent']
    df_g = normalize_identifiers(df_g)
    dps_norm = normalize_identifiers(dps_cfg[cols].copy())
    m = df_g.merge(dps_norm, on=['material', 'location'], how='left')
...
def apply_supply_choice(
    ...
    # 合并并应用调整（归一化后再 merge，避免 material 类型不匹配）
    df_g = normalize_identifiers(df_g)
    sup_g = normalize_identifiers(sup_g)
