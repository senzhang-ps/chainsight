"""Build the FEM baseline KPI analysis deliverable.

Project : fem-cs-test
Scenario: baseline-fem-hpfd-network-0386
run_id  : db_baseline-fem-hpfd-network-0386_20260602_223559   (DB: fem_test)
...
Run from repo root with the venv interpreter:
  .\\.venv\\Scripts\\python.exe .\\workspace\\fem-cs-test\\scenarios\\baseline-fem-hpfd-network-0386\\analysis\\build_fem_analysis.py
"""
...
CONFIG_NAME = "baseline-fem-hpfd-network-0386"
DBNAME = "fem_test"
LINE = "HPFD"
...
RUN_TAG = datetime.now().strftime("%Y%m%d-%H%M") + "-fem-kpi"
...
def pull_postgres():
    ...
    return (orders, ships, changeover, prod_plan, prd_rate, prod_runs,
            stock_me, fcst_weekly, sorted(hpfd_mats_df["material"].tolist()))
...
def compute_deepdive(changeover, prod_runs, su_factor):
    ...
    info = {"pkg_ids": ", ".join(sorted(PKG_IDS)), "conv_ids": ", ".join(sorted(CONV_IDS)),
            "total_msu": round(total_msu, 2),
            "note": "changeover id '2-1' not present in this run (0 events); '2-11' present"}
...
def main():
    ...
    (orders, ships, changeover, prod_plan, prd_rate, prod_runs,
     stock_me, fcst_weekly, hpfd_mats) = pull_postgres()
    ...
    html = build_html(tof_loc, tof_month, tof_piv, co_by_id, co_cnt_piv, co_by_month,
                      ...
                      deep, deep_month, deep_info, su_info, charts)
    (OUT_DIR / "analysis.html").write_text(html, encoding="utf-8")

    print("[6/6] writing run.md + LATEST.md ...")
    write_run_md(tof_month, co_by_month, dfc_month, ptt, apq_info, deep, su_info, dfc_info)
    print(f"\nDONE -> {OUT_DIR}")
...
def build_html(tof_loc, tof_month, tof_piv, co_by_id, co_cnt_piv, co_by_month,
    ...
    <title>FEM Baseline KPI Analysis</title><style>{css}</style></head><body><div class="wrap">
    <h1>FEM &times; HPFD Baseline &mdash; KPI Analysis</h1>
    ...
    <div class="note">All {22} materials ordered at the scope DCs are HPFD-line materials, so the network is fully HPFD-scoped.
    ...
    runs via the campaign rule ({apq_info['base_runs']} base-product campaign(s) in scope &mdash; e.g. 80799574 ran
    Feb&nbsp;10&ndash;11 and again Mar&nbsp;12) and carry {apq_info['base_msu']:,.1f} MSU
...
def write_run_md(tof_month, co_by_month, dfc_month, ptt, apq_info, deep, su_info, dfc_info):
    r = deep.iloc[0]
    lines = [
        f"# FEM baseline KPI analysis - run {RUN_TAG}",
        "",
        f"- level: scenario",
        f"- project: {PROJECT}",
        f"- scenario: {SCENARIO}",
    ...
    lines += [
        ...
        f"rule ({apq_info['base_runs']} base-product campaign(s) in scope, e.g. 80799574 = Feb 10-11 + Mar 12). They carry "
