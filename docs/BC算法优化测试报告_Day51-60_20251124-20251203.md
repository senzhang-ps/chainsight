...
| **Src** | 650秒 (10.8分钟) | 65.0秒 |
| **DB** | 441秒 (7.3分钟) | 44.1秒 |

### 2.2 性能提升比例
| 对比项 | 加速倍数 | 时间减少百分比 |
|--------|----------|----------------|
| **Src vs Dev** | **1.74x** | 42.5% |
| **DB vs Dev** | **2.56x** | 61.0% |
| **DB vs Src** | **1.47x** | 32.2% |

### 2.3 各模块平均耗时对比（秒/天）
| 模块 | Dev | Src | DB | Src加速 | DB加速 |
|------|-----|-----|-----|---------|--------|
| **M1 订单生成** | ~19.3 | ~13.1 | ~15.8 | 1.47x | 1.22x |
| **M3 净需求计算** | ~24.4 | ~10.0 | ~5.0 | 2.44x | 4.88x |
| **M5 部署规划** | ~61.4 | ~34.0 | ~14.5 | 1.81x | 4.23x |

### 2.4 每日详细耗时（区间最后一天）
#### Day 60 (2025-12-03) 各模块耗时（秒）
| 模块 | Dev | Src | DB |
|------|-----|-----|-----|
| M1 订单生成 | ~25.0 | ~14.0 | ~16.0 |
| M3 净需求计算 | ~27.0 | ~10.0 | ~5.0 |
| M5 部署规划 | ~79.0 | ~35.0 | ~15.0 |
| M6 物流执行 | ~2.0 | ~1.0 | ~1.0 |
...
| 指标 | Dev | Src | DB | 最佳提升 |
|------|-----|-----|-----|----------|
| 区间总时间 | 18.9分钟 | 10.8分钟 | 7.3分钟 | 61.0% |
| M1 订单生成平均耗时/天 | ~19.3秒 | ~13.1秒 | ~15.8秒 | 32.1% |
| M3 净需求计算平均耗时/天 | ~24.4秒 | ~10.0秒 | ~5.0秒 | 79.5% |
| M5 部署规划平均耗时/天 | ~61.4秒 | ~34.0秒 | ~14.5秒 | 76.4% |
