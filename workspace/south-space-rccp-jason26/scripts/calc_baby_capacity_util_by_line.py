...
with psycopg.connect(
    ...
    prod = pd.read_sql_query(
        """
        select simulation_date::date as date, location, line, material,
               produced_qty, changeover_time
...
prod['changeover_time'] = pd.to_numeric(prod['changeover_time'], errors='coerce').fillna(0)
...
# user-defined logic: total production time including changeover time
# using productionplan changeover_time at Baby material rows + aggregated changeoverlog time
prod_daily = prod.groupby(['date','location','line'], as_index=False).agg(
    production_time=('produced_qty','sum'),
    embedded_changeover_time=('changeover_time','sum')
)
...
base['embedded_changeover_time'] = base['embedded_changeover_time'].fillna(0)
...
out_path = RESULTS / 'baby_by_month_by_line_capacity_utilization.csv'
