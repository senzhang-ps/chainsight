...
# -*- coding: utf-8 -*-
"""
generate_oc_10day_reports.py

Parse three simulation logs (Dev, Src, DB) and the 3-way content comparison
JSON to generate markdown reports for each 10-day interval.
...
Output goes to  docs/  with filenames like:
    OC算法优化测试报告_Day01-10_20251215-20251224.md

Usage:
    python tools/generate_oc_10day_reports.py
"""
...
def generate_interval_report(
    ...
    # Section 1: 测试概述
    # ═══════════════════════════════════════════════════════════════
    a(f"# ChainSight 供应链仿真系统 OC算法优化测试报告 (Day {day_label_start}-{day_label_end})")
    a("")
    a("## 1. 测试概述")
    a("")
    a("### 1.1 测试目的")
    ...
    a("- **仿真周期**: 76天 (2025-12-15 至 2026-02-28)")
    ...
    for mod_label, mod_cn, tables in MODULE_TABLES:
        ...
        for tkey, tcn in tables:
            ...
            a("")
            a(
                f"- **总行数 (全76天)**: Dev={tbl_data['total_dev_rows']:,}, "
            ...
            # Show diff details if any FAIL exists for dev_vs_db on module1/Summary
            fail_days_db = [
                dd for dd in interval_days
                if not dd["dev_vs_db"]["content_match"]
                and "diff_details" in dd["dev_vs_db"]
            ]
            if fail_days_db and tkey == "module1/Summary":
                a(
                    "> **差异说明**: `total_orders` 列 -- DB版本记录当日新增订单数，"
                    "Dev/Src版本记录截至当日的累积订单数。该差异为统计口径不同，"
                    "不影响任何实际业务数据的正确性。"
                )
                a("")
...
def main() -> None:
    ...
    for ds, de in INTERVALS:
        ...
        fname = f"OC算法优化测试报告_Day{ds:02d}-{de:02d}_{d_start_str}-{d_end_str}.md"
    ...
    print("\n=== Quick Performance Summary (full 76 days) ===")
