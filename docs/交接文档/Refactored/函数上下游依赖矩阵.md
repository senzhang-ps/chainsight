...
| 版本 | 日期 | 修订人 | 修订说明 |
|---|---|---|---|
| v1.0 | 2026-03-06 | 待填写 | 首次形成上下游依赖矩阵和排障速查 |
...
|---|---|---|---|---|
| 1 | `src/core/run.py::main()` | CLI 参数、配置路径、DB 参数 | 运行模式选择、本次 run 目录 | `outputs/<config>/run_*` |
| 2 | `src/core/main_integration.py::run_integrated_simulation()` | Excel 配置 / `config_dict` | 启动 Orchestrator、按天驱动模块 | 各模块日输出 + Orchestrator 状态 |
...
| 4 | `src/core/orchestrator.py::process_module1_shipments()` | M1 `shipment_df` | 扣库存、记录 shipment log | `shipment_log_YYYYMMDD.csv` |
| 5 | `src/core/main_integration.py::run_module4_integrated()` | M4 配置、M3 净需求 | `production_df/exceed_log/issues_df/changeover_log` | `Module4Output_YYYYMMDD.xlsx` |
| 6 | `src/core/orchestrator.py::process_module4_production()` | M4 `production_df` | backlog、production GR、增加库存 | `production_gr_YYYYMMDD.csv` / `production_plan_backlog_YYYYMMDD.csv` |
...
| 8 | `src/core/orchestrator.py::process_module5_deployment()` | M5 `deployment_plan` | 写 open deployment + UID | `open_deployment_YYYYMMDD.csv` |
| 9 | `src/modules/module6.py::run_daily_physical_flow()` | OpenDeployment、M6 配置、当前库存 | `delivery_plan/vehicle_log/truck_usage/unsatisfied_log` | `Module6Output_YYYYMMDD.xlsx` |
| 10 | `src/core/orchestrator.py::process_module6_delivery()` | M6 `delivery_plan` | 扣 open deployment、扣发送库存、写在途或 delivery GR | `planning_intransit_YYYYMMDD.csv` / `delivery_gr_YYYYMMDD.csv` |
...
| 12 | `src/core/orchestrator.py::save_daily_state()` | 当日全部状态 | 全量状态快照 | `orchestrator/*.csv` |
...
| 函数 | 上游依赖 | 下游依赖 | 文件/表影响 |
|---|---|---|---|
| `main()` | CLI 参数 | `_ensure_output_dir`, `run_integrated_simulation`, `_run_with_database` | 选择本地或 DB 模式 |
...
### 3.2 `src/core/main_integration.py`
