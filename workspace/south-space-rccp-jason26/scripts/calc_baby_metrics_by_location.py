...
LOCATIONS = ['C816', 'C810', 'D873', 'C866', 'C867']
CATEGORY = 'Baby'
...
mat_set = set(mapping.loc[mapping['category_en'] == CATEGORY, 'material'])
...
with psycopg.connect(
    ...
    inv = pd.read_sql_query(
        ...
        """,
        conn,
        params={'run_id': RUN_ID, 'locations': LOCATIONS}
    )
...
inv_f = inv[inv['material'].isin(mat_set)].copy()
ord_f = ordlog[ordlog['material'].isin(mat_set)].copy()
shp_f = shplog[shplog['material'].isin(mat_set)].copy()
fc_f = fc[fc['material'].isin(mat_set)].copy()
...
out_path = RESULTS / 'baby_by_month_by_location_rccp_peakdfc_service.csv'
