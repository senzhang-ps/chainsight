...
| **Src** | 1986秒 (33.1分钟) | 198.6秒 |
| **DB** | 1081秒 (18.0分钟) | 108.1秒 |

### 2.2 性能提升比例
| 对比项 | 加速倍数 | 时间减少百分比 |
|--------|----------|----------------|
| **Src vs Dev** | **6.45x** | 84.5% |
| **DB vs Dev** | **11.86x** | 91.6% |
| **DB vs Src** | **1.84x** | 45.6% |

### 2.3 各模块平均耗时对比（秒/天）
| 模块 | Dev | Src | DB | Src加速 | DB加速 |
|------|-----|-----|-----|---------|--------|
| **M1 订单生成** | ~378.5 | ~38.8 | ~38.0 | 9.76x | 9.96x |
| **M3 净需求计算** | ~340.2 | ~44.8 | ~20.6 | 7.59x | 16.51x |
| **M5 部署规划** | ~541.6 | ~105.2 | ~40.5 | 5.15x | 13.37x |

### 2.4 每日详细耗时（区间最后一天）
#### Day 40 (2026-01-23) 各模块耗时（秒）
| 模块 | Dev | Src | DB |
|------|-----|-----|-----|
| M1 订单生成 | ~383.0 | ~39.0 | ~37.0 |
| M3 净需求计算 | ~341.0 | ~45.0 | ~20.0 |
| M5 部署规划 | ~552.0 | ~102.0 | ~38.0 |
| M6 物流执行 | ~3.0 | ~1.0 | ~1.0 |
...
|------|------|---------|---------|--------|------------|-----------|-----------|
| Module1 | 订单日志 (OrderLog) | 46,352 | 46,352 | 46,352 | PASS | PASS | PASS |
...
| **合计** | | **46,352** | **46,352** | **46,352** | **10/10天PASS** | **10/10天PASS** | **10/10天PASS** |
...
验证覆盖（本区间）：
- Module1 / 订单日志 (OrderLog): 46,352行 (Dev vs Src PASS, Dev vs DB PASS, Src vs DB PASS)
...
| 指标 | Dev | Src | DB | 最佳提升 |
|------|-----|-----|-----|----------|
| 区间总时间 | 213.7分钟 | 33.1分钟 | 18.0分钟 | 91.6% |
| M1 订单生成平均耗时/天 | ~378.5秒 | ~38.8秒 | ~38.0秒 | 90.0% |
| M3 净需求计算平均耗时/天 | ~340.2秒 | ~44.8秒 | ~20.6秒 | 93.9% |
| M5 部署规划平均耗时/天 | ~541.6秒 | ~105.2秒 | ~40.5秒 | 92.5% |
