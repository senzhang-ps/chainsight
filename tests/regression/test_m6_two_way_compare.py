...
def test_m6_legacy_pandas_polars_replay_five_real_business_days(real_replay):
    # 测试目的：验证“m6、legacy、pandas、polars、replay、five、real、business、days”场景下物流执行、车辆分配与交付结果的一致性。
...
def test_m6_real_replay_records_comparable_execution_timing(real_replay):
    # 测试目的：验证“m6、real、replay、records、comparable、execution、timing”场景下物流执行、车辆分配与交付结果的一致性。
    # 测试方法：准备该场景所需的夹具、配置或模拟数据，调用 `all()`，再通过 3 个断言核对结果、状态或异常条件。
    # 输入和期望：输入为测试构造的正常、边界或异常业务数据；期望结果满足本用例的断言契约。
    # 业务逻辑和原因：该校验防止物流执行、车辆分配与交付结果的一致性在重构、引擎切换或跨日运行时发生静默偏差。
