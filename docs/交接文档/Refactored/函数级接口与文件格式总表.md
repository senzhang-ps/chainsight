...
| 版本 | 日期 | 修订人 | 修订说明 |
|---|---|---|---|
| v1.0 | 2026-03-06 | 待填写 | 首次形成函数级接口、文件格式、关键复杂文件深度说明 |
...
1. `run.py`
2. `src/core/run.py::main()`
3. `src/core/main_integration.py::run_integrated_simulation(...)`
...
1. `run.py`
2. `src/core/run.py::main()`
3. `src/core/run.py::_run_with_database(...)`
...
### 5.2 `src/core/main_integration.py`
...
### 16.1 `src/core/main_integration.py`
...
#### `load_module4_production_output(...)`
- 路径：兼容/辅助路径
- 作用：回读单个 M4 输出文件，常用于兼容历史调用或调试。
...
### 21.1 `main.py`（原 module6.py，v2.0 已拆分至子包）
...
- `OptimizationConfig`：统一优化开关。
- `should_use_duckdb`：是否建议走 DuckDB。
- `use_duckdb_for_merge/groupby/filter`：针对具体操作的策略函数。

#### `parallel_optimizer.py`
- `parallel_batch_process`：通用批处理并行。
- `batch_dataframe_filter`：批量过滤 DataFrame。
- `vectorized_demand_aggregation`：向量化聚合。
- `ParallelBatchProcessor`：批量并行器。
- `precompute_all_indices`：统一预建索引。

#### `process_pool_executor.py`
- `batch_process_parallel`：进程池批处理。
- `parallel_dataframe_groupby_apply`：DataFrame 分组并行。
- `parallel_layer_process`：层级任务并行。
- `ProcessPoolManager`：全局进程池管理器。

#### `multiprocess_executor.py`
- `MultiProcessExecutor`：通用多进程执行器。
- `parallel_dataframe_apply`：DataFrame 分块并行 apply。

#### `high_perf_executor.py`
- `HighPerformanceExecutor`：更高层的高性能执行框架。
- `parallel_process_layers`：层级处理入口。
- `optimize_cpu_bound_loop`：CPU 密集循环优化。
...
#### `performance.py`
- `normalize_*_vectorized`：向量化标准化。
- `ConfigCache`：预建配置查询缓存。
- `efficient_merge`：更稳妥的大表 merge。
- `batch_groupby_apply`：分批 groupby 应用。

#### `duckdb_sql_wrapper.py`
- `DuckDBSQL`：把常用 merge/groupby/filter 封装到 DuckDB。
- `smart_merge/smart_groupby_agg/smart_filter`：自动选择策略。
...
> v2.0 变更：原有的 `module1.py`~`module6.py` 兼容层文件已删除。模块导出统一通过 `src/modules/__init__.py` 的向后兼容别名实现。

### `src/modules/__init__.py`
- 作用：统一导出所有模块子包，并提供 `module1 = demand_planning` 等向后兼容别名。
- 导出子包：`demand_planning`、`mrp_planning`、`production_planning`、`deployment_planning`、`logistics_execution`。

### `src/modules/demand_planning/`
- 重点公开 API：`run_daily_order_generation`, `generate_daily_orders`, `generate_shipment_with_inventory_check`, `expand_forecast_to_days_integer_split`, `apply_dps`, `apply_supply_choice`, `load_config`。

### `src/modules/mrp_planning/`
- 重点公开 API：`run_integrated_mode`, `run_mrp_layered_simulation_daily`, `calculate_daily_net_demand`, `assign_location_layers`, `determine_lead_time`。

### `src/modules/production_planning/`
- 重点公开 API：`run_daily_production_planning`, `DailyProductionPlanner`, `build_unconstrained_plan_for_single_day`, `centralized_capacity_allocation_with_changeover`, `simulate_production`。

### `src/modules/deployment_planning/`
- 重点公开 API：`main`, `load_integrated_config`, `collect_node_demands`, `apply_grouped_moq_rv`, `apply_priority_allocation_vectorized`, `push_softpush_allocation`。

### `src/modules/logistics_execution/`
- 重点公开 API：`run_daily_physical_flow`, `run_physical_flow_module`。
- v2.0 已拆分为 8 个子模块：`main.py`, `initializer.py`, `data_preparer.py`, `simulation_engine.py`, `route_processor.py`, `constraint_enforcer.py`, `output_builder.py`, `constants.py`。
...
- v2.0 已删除 `module1.py`~`module6.py` 兼容层文件，所有逻辑都在对应子包中。
- `logistics_execution/` 子包在 v2.0 中从单文件拆分为 8 个子模块，维护优先级较高。
- 如果你要进一步把这份手册升级成”逐文件逐函数完全展开版”，下一步最值得继续深挖的是：
  1. `src/core/orchestrator/orchestrator_main.py`
  2. `src/modules/deployment_planning/main.py`
  3. `src/modules/production_planning/capacity_allocator.py`
  4. `src/modules/logistics_execution/main.py`
...
### 28.1 `src/core/orchestrator.py` 深度说明
...
### 28.4 `src/modules/logistics_execution/main.py` 深度说明（原 module6.py）
...
1. `orchestrator.py` 里的标识符标准化和 UID 逻辑不能局部绕过。
2. `orchestrator.py` 的 `_process_delivery_arrivals()` 必须保持在日初执行。
...
5. `logistics_execution/` 中 route 顺序、truck type 顺序、第一轮/第二轮装载、UID 去重顺序都会影响最终结果。
