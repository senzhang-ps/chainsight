...
使用：
    ...
    m1_result, m4_result, m5_result = executor.run_parallel_stage(
        tasks=[
            ('Module1', m1_task_fn),
            ('Module4', m4_task_fn),
            ('Module5', m5_task_fn),
...
class ParallelTaskResult:
    ...
    def __str__(self) -> str:
        ...
        if self.status == 'success':
            return (f"✅ {self.task_name}: 成功 ({self.elapsed_time:.2f}s)")
        elif self.status == 'error':
            return (f"❌ {self.task_name}: 失败 - {str(self.error)}")
        else:
...
class ParallelExecutor:
    ...
    Args:
        ...
        enable_parallel: 是否启用并行（默认True，可通过CHAINSIGHT_PARALLEL环境变量覆盖）
    """
    ...
    def __init__(self, max_workers: int = 3, enable_parallel: Optional[bool] = None):
        ...
        if enable_parallel is None:
            enable_parallel = os.getenv('CHAINSIGHT_PARALLEL', 'true').lower() == 'true'
        
        self.max_workers = max_workers
        self.enable_parallel = enable_parallel
        self.executor = ThreadPoolExecutor(max_workers=max_workers)
        
        logger.info(f"🔄 并行执行器已初始化 - max_workers={max_workers}, "
    ...
    def run_parallel_stage(
        ...
        for task_name, task_fn in tasks:
            future = self.executor.submit(self._run_single_task, task_name, task_fn)
    ...
    def _print_results(
        ...
        if parallel:
            # 并行模式：总耗时 = max(各任务耗时)
            max_time = max((r.elapsed_time for r in results.values()), default=0)
...
def run_parallel_modules(
    ...
    """
    executor = ParallelExecutor(max_workers=max_workers, enable_parallel=enable_parallel)
