...
| **Src** | 2031秒 (33.9分钟) | 203.1秒 |
| **DB** | 1114秒 (18.6分钟) | 111.4秒 |

### 2.2 性能提升比例
| 对比项 | 加速倍数 | 时间减少百分比 |
|--------|----------|----------------|
| **Src vs Dev** | **6.45x** | 84.5% |
| **DB vs Dev** | **11.77x** | 91.5% |
| **DB vs Src** | **1.82x** | 45.2% |

### 2.3 各模块平均耗时对比（秒/天）
| 模块 | Dev | Src | DB | Src加速 | DB加速 |
|------|-----|-----|-----|---------|--------|
| **M1 订单生成** | ~387.1 | ~38.5 | ~37.8 | 10.05x | 10.24x |
| **M3 净需求计算** | ~347.1 | ~45.6 | ~21.5 | 7.61x | 16.14x |
| **M5 部署规划** | ~564.2 | ~113.0 | ~45.8 | 4.99x | 12.32x |

### 2.4 每日详细耗时（区间最后一天）
#### Day 30 (2026-01-13) 各模块耗时（秒）
| 模块 | Dev | Src | DB |
|------|-----|-----|-----|
| M1 订单生成 | ~376.0 | ~38.0 | ~40.0 |
| M3 净需求计算 | ~342.0 | ~44.0 | ~21.0 |
| M5 部署规划 | ~557.0 | ~108.0 | ~45.0 |
| M6 物流执行 | ~5.0 | ~3.0 | ~3.0 |
...
|------|------|---------|---------|--------|------------|-----------|-----------|
| Module1 | 订单日志 (OrderLog) | 46,284 | 46,284 | 46,284 | PASS | PASS | PASS |
...
| **合计** | | **46,284** | **46,284** | **46,284** | **10/10天PASS** | **10/10天PASS** | **10/10天PASS** |
...
验证覆盖（本区间）：
- Module1 / 订单日志 (OrderLog): 46,284行 (Dev vs Src PASS, Dev vs DB PASS, Src vs DB PASS)
...
| 指标 | Dev | Src | DB | 最佳提升 |
|------|-----|-----|-----|----------|
| 区间总时间 | 218.5分钟 | 33.9分钟 | 18.6分钟 | 91.5% |
| M1 订单生成平均耗时/天 | ~387.1秒 | ~38.5秒 | ~37.8秒 | 90.2% |
| M3 净需求计算平均耗时/天 | ~347.1秒 | ~45.6秒 | ~21.5秒 | 93.8% |
| M5 部署规划平均耗时/天 | ~564.2秒 | ~113.0秒 | ~45.8秒 | 91.9% |
